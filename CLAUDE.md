...
| TP/SL на бинарных рынках | Только в пунктах вероятности (`take_profit_delta`/`stop_loss_delta`), не в % от цены: при входе по 0.9 «+25%» недостижимы |
...
**Управление капиталом (`src/tracker.py`):** размер позиции — fractional Kelly `position_size_usd()` (0.25 Келли, кап 2% банкролла); лимиты `entry_blocked()` — максимум 10 позиций, одна на рынок; дневной стоп `daily_stop_active()` — просадка ≥ 5% за день UTC → пауза входов до завтра (состояние в `data/metrics.json`); флиппинг — при +5 центов фиксируется 50% позиции, остаток едет до TP +10 / SL −15.
